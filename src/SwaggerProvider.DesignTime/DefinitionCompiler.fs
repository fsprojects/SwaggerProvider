...
type NamespaceAbstraction (name:string) =
    let providedTys = Collections.Generic.Dictionary<_,_>()

    member __.Name = name
    member __.ChooseAndReserveUniqueName namePref nameSuffix =
        ...
        newName
    member __.TryGetType tyName =
        providedTys.TryGetValue tyName
    member __.Register (tyName,value) =
        match providedTys.ContainsKey tyName with
        | true -> providedTys.[tyName] <- Some value
        | _ -> providedTys.Add(tyName, Some value)
    member __.GetRegisteredTypes() =
...
type DefinitionCompiler (schema:SwaggerObject, provideNullable) as this =
    ...
    and compileSchemaObject tyName (schemaObj:SchemaObject) isRequired =
        let compileNewObject (properties:DefinitionProperty[]) =
            if properties.Length = 0 then typeof<obj>
              ...
              if isNull tyName then
                ...
                match ns.TryGetType tyName with
                | true, Some(ty) -> ty :> Type
                | _ ->
                    let tyName = ns.ChooseAndReserveUniqueName tyName ""
                    let ty = ProvidedTypeDefinition(tyName, Some typeof<obj>, isErased = false)
                    ns.Register(tyName,ty)
                    ...
                    let members =
                        ...
                        |> List.map (fun p ->
                            ...
                            let pTy = compileSchemaObject (ns.ChooseAndReserveUniqueName tyName (nicePascalName p.Name)) p.Type p.IsRequired
        ...
        let tyType =
            ...
            | Array eTy       -> (compileSchemaObject (ns.ChooseAndReserveUniqueName tyName "Item") eTy true).MakeArrayType()
            | Dictionary eTy  -> ProvidedTypeBuilder.MakeGenericType(typedefof<Map<string, obj>>, 
                                        [typeof<string>; compileSchemaObject (ns.ChooseAndReserveUniqueName tyName "Item") eTy false])
    ...
    member __.GetProvidedTypes() =
        ns.GetRegisteredTypes()

    /// Compiles the definition.
    member __.CompileTy opName tyUseSuffix ty required =
        compileSchemaObject (ns.ChooseAndReserveUniqueName opName tyUseSuffix) ty required
