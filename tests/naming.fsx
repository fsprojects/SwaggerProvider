...
let loadSchema (url:string) =
    try
        ...
        |> Some
    with
    | :? Net.WebException -> None

let analyze schemas =
    let schemas = 
        schemas |> Array.choose (fun url ->
            printfn "Loading '%s'..." url
            loadSchema url |> Option.map(fun x-> (url, x)) )
    let toCsv (arr:string[]) = sprintf "\"%s\"" <| String.Join("\",\"", arr)
    let definitions =
        schemas 
        |> Array.collect (fun (url, schema) ->
            ...
            |> Array.map (fun (name, _) ->
                let tyName = 
                    ...
                    |> nicePascalName |> scope.MakeUnique
                toCsv [| name; tyName; url |]
            )
        )
    let paths =
        schemas |> Array.collect (fun (url, schema) ->
            let scope1 = UniqueNameGenerator()
            let scope2 = UniqueNameGenerator()
            schema.Paths
            |> Array.map (fun path ->
                toCsv [| path.OperationId;
                   scope1.MakeUnique <| OperationCompiler.GetMethodNameCandidate path true; 
                   scope2.MakeUnique <| OperationCompiler.GetMethodNameCandidate path false;
                   path.Path; url |]
            )
        )
    definitions, paths
...
let getPath name = Path.Combine( __SOURCE_DIRECTORY__ ,name)
